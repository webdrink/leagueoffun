...
const IntroScreen: React.FC<IntroScreenProps> = ({
  ...
  className="w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl p-6 bg-white/90 backdrop-blur-sm shadow-xl rounded-2xl border-2 border-rust-100"
    >      <div className="text-center mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-autumn-700">{t('intro.heading')}</h1>
        <p className="text-rust-600 mt-2 text-sm sm:text-base">{t('intro.subheading')}</p>
      ...
      <div className="mt-6 flex flex-col space-y-4">
        <Button
          ...
          disabled={isLoading}
          className="w-full bg-autumn-600 hover:bg-autumn-700 text-white py-3 text-lg rounded-lg shadow-md transition-transform hover:scale-105 duration-200 disabled:opacity-70 disabled:cursor-not-allowed"
        >
        ...
        <div className="flex items-center justify-between pt-2">
          <Label htmlFor="nameBlameModeToggle" className="flex items-center cursor-pointer select-none">
            <Switch
              ...
              className="data-[state=checked]:bg-rust-500"
            />
            <span className="ml-2 text-sm text-autumn-700">{t('intro.name_blame_toggle')}</span>
          </Label>
        </div>
        
        {nameBlameMode && (
          <div className="mt-3 p-3 bg-rust-50 border border-rust-200 rounded-lg">
            <p className="text-xs text-autumn-600 leading-relaxed">
          ...
          <div className="flex items-center justify-between pt-2">
            <Label htmlFor="categorySelectToggle" className="flex items-center cursor-pointer select-none">
              <Switch
                ...
                className="data-[state=checked]:bg-autumn-500"
              />
              <span className="ml-2 text-sm text-autumn-700">{t('intro.select_categories')}</span>
            </Label>
          </div>
        )}
      </div>
        <div className="mt-4 pt-4 border-t border-rust-200">
        <VolumeControl 
          volume={volume} 
